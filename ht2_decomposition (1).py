...
'''
Computes an approximation of the left singular vectors and singular values of a matrix 𝑋 using QR decomposition followed by Singular Value Decomposition (SVD). 
It keeps only the top rank singular vectors and values.

QR Decomposition: Transposes 𝑋 and computes an upper triangular matrix 𝑅
SVD: Applies SVD to 𝑅 transpose to extract singular vectors (𝑈) and singular values (𝑆).
Truncation: Returns the first rank components of 𝑈 and 𝑆
'''
...
#QR and SVD and return the singular values only
